/*
* Funcion JavaScript que envia datos plano a través del servidor web
*/
	...
	if(entorno=="ligero")
	{
		if(opNACAR != null && opNACAR !="" && opNACAR!="null") 
		{
			...
			if(portal!=null)
				...
				alert("No se ha ejecutado la operación porque no se ha establecido comunicación con el Portal");
		}
		else
		{
			var portal=utils_servidorWeb_obtenerPortal(window);
...
// ---------------------------------------------------------------------------
// Función que se ejecuta cuando no se ha especificado una función por defecto//
// ---------------------------------------------------------------------------
...
servidorweb.utils_servidorWeb_tratarF5=function (puerto,entorno,ip,claveseguridad)
{
	...
	if(entorno=="ligero")
	{
		...
		if(portal!=null)
		{
			...
			alert("No se ha realizado la bajada del objeto de negocio porque no se ha establecido comunicación con el Portal");		
...
servidorweb.utils_servidorWeb_subirObjetosNegocio=function (claves, valores,port,entorno,sincAsinc,ip,claveseguridad) {
	...
	if(entorno=="ligero")
	{
		...
		if(portal!=null)
            ...
            alert("No se han subido los datos al objeto de negocio porque no se ha establecido comunicación con el Portal");		
	}
	else
	{
...
/**********************************************************************************
 * Método que se encarga de procesar la respuesta devuelta por el servidor web
 *********************************************************************************/  
...
/****************************
 * Método ejecutado por el Servidor Web tras recibir un mensaje desde Júpiter. Internamente a la función, se realizará una llamada 
 * a una función JS aplicativa que será la que realice el tratamiento del mensaje "aplicacion_procesarRespuestaJupiter".
 ***************************/
servidorweb.utils_servidorweb_procesarRespuestaJupiter=function (mensaje){
  ...
  if(mensaje!=undefined && mensaje!=null && mensaje!=""){
    ...
    // Se llama al método aplicativo que realiza el tratamiento de documento xml.
...
/****************************
 * Realiza el envio de datos a Júpiter. Es decir, deberá ser ejecutada por las aplicaciones ligeras cuando deseen enviar datos a Júpiter.
 * Se enviarán los datos del formulario NACAR formateados en un mensaje XML. Para ello, se apoyará en la función JavaScript
 * utils_servidorweb_componerContextoJupiter.
 * Adicionalmente, se dará la posibilidadad de activar o no la tarea cuando esta reciba los datos. Por defecto no se activará. 
 * Además, se permitirá mediante un valor booleano la posibilidad de lanzar la petición de forma síncrona o, de forma asíncrona.
 * Por defecto, la petición se realizará de forma asíncrona.      
 ***************************/
...
// Si se trata de una petición síncrona, se realiza el bloqueo de la ventana. 
...
/****************************
 * Inicializa un diálogo que se mostrará sobre la ventana con el fín de bloquear la interacción con esta.
 ***************************/
...
/****************************
 * Muestra un diálogo que bloquea la interacción con la aplicación ligera.
 ***************************/
...
/****************************
 * Oculta el diálogo que bloquea la interacción con la aplicación ligera.
 ***************************/
...
/****************************
 * Recorrerá los campos del formulario de la aplicación ligera comprobando los elementos que se corresponden con valores
 * de contexto para formar el xml de datos correspondiente.
 * Esta función será privada a la librería, por lo que las aplicaciones no tendrán que realizar su invocación.       
 ***************************/
...
/****************************
 * Recorrerá los campos del formulario de la aplicación ligera y, compondrá un mensaje XML con los datos de la aplicación de la forma:
...
{  
  ...
  try{
    if (utils_isComuServidorWeb()){  
		//Si utilizamos el servidorWeb inicia la comunicación    
...
/*Funcion de respuesta vacia utilizada para conseguir mantener el valor del parámetro sincAsinc en una petición AJAX, pues por defecto, 
 *si la función respuesta es null o no está definida el parámetro sincAsinc pasa a tener valor false (síncrno)
*/
...
function utils_servidorWeb_detenerComunicacionPersistente (puerto,idPeticion){ 
  ...
  var parametros=new Array(puerto,idPeticion);
  //Se realiza una petición sincrona
...
/****************************
 * Función que realizará el tratamiento del mensaje XML recibido desde Jupiter. Debera ser codificado desde la aplicación.
 * Se deberán basar en funciones de Arquitectura para realizar el tratamiento, como acceder a determinados campos de mensaje o formatearlos.
 ***************************/ 
...
/****************************
 * Función que retorna la estructura de contexto asociada a una determinada clave. Por ejemplo, para la clave PAR.DATOS_SALIDA.USUARIO
 * puede devolver el código de un determinado usuario, o por el contrario puede devolver la estructura asociada a éste.
 * -USUARIO
 *      -NOMBRE
 *      -APELLIDOS
 *      -...   
...
/****************************
 * Función que transforma a un dato de tipo String la estructura de contexto XML pasada por parámetro.
 ***************************/ 
...
// ----------------------------------------------------------------------------------------
// Función que convierte a String la cabecera de una determinada etiqueta del documento XML//
// ----------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------
// Función que convierte a String el final de una determinada etiqueta del documento XML//
// -------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------------------------------
// Función que recupera el texto de un item.
// -------------------------------------------------------------------------------------------------------------
...
function utils_servidorWeb_ejecutarTareaObjetoNegocio(nombre_tarea,url,urlDesconexion,tipo_tarea,objCliente) 
{
	...
	if(portal!=null)
		...
		alert("No se ha ejecutado la tarea porque no se ha establecido comunicación con el Portal");		
}
...
function utils_servidorWeb_activarTarea(idTarea,argumentos)
{
	...
	if(portal!=null)
		...
		alert("No se ha activado la tarea porque no se ha establecido comunicación con el Portal");		
}
//Destruye tareas
function utils_servidorWeb_destruirTarea(idTarea,argumentos)
{
	...
	if(portal!=null)
		...
		alert("No se ha destruido la tarea porque no se ha establecido comunicación con el Portal");		
}
...
function utils_servidorWeb_destruirTareasPorEscenario(codEscenario,argumentos)
{
	...
	if(portal!=null)
		...
		alert("No se han destruido las tareas porque no se ha establecido comunicación con el Portal");		
}
...
function utils_servidorWeb_modificarDescripcionAplicativa(idTarea,descripcion,argumentos)
{
	...
	if(portal!=null)
		...
		alert("No se ha modificado la descripción de la tarea porque no se ha establecido comunicación con el Portal");		
...
function utils_servidorWeb_getIDTareaActiva(argumentos)
{
	...
	if(portal!=null)
		...
		alert("No se ha obtenido el identificador de la tarea porque no se ha establecido comunicación con el Portal");		
}
//Obtiene el número de tareas abiertas
function utils_servidorWeb_get_numero_tareas_abiertas()
{
	...
	if(portal!=null)
		...
		alert("No se ha obtenido el número de tareas activas porque no se ha establecido comunicación con el Portal");		
}
//Activa un escenario
function utils_servidorWeb_activarEscenario(codEscenario) 
{
	...
	if(portal!=null)
		...
		alert("No se ha activado el escenario porque no se ha establecido comunicación con el Portal");		
}
...
function utils_servidorWeb_obtenerObjetoNegocio() 
{
	...
	{
		alert("No se ha recuperado el objeto de negocio porque no se ha establecido comunicación con el Portal");	
		return null;
	}
}
//Obtiene el número máximo de padres que se buscarán para llegar a la ventana del portal
function utils_servidorWeb_getNumMaxParent()
{
    return NUM_MAX_PARENT;
}
//Establece el número máximo de padres que se buscarán para llegar a la ventana del portal
...
/**
 * Función utilizada para utilizar el servicio de Mensajes de Arquitectura
 */ 
function utils_servidorWeb_EscribirMensaje(maquina,puerto,servlet,claveSeguridad,codigoMensaje, tipoMensaje, tipoDialogo, backEnd, mensaje){
  ...
  var datosMensaje={"codigoMensaje": codigoMensaje, 
...
// Función que realiaza una llamada al serlet GUC para mostrar la ventana principal de los mensajes GUC
