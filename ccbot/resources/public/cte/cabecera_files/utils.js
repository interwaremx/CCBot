...
/*Si nos encontramos en IE8 Standard se a�ade al plugin un style
		...
		if (document.documentMode != undefined && document.documentMode != null) {
			if (document.documentMode == 8) {
				...
				if (objPlugin != undefined && objPlugin != null && objPlugin != "") {
					//Se a�ade el style para que funcione correctamente los botones del mensaje dojo
...
//Variable que indica si se est� lanzando un mensaje error desde un frame hijo
...
function obtenerContenedor(flujoId) {
	...
	try {
		//Se comprueba si se est� lanzando el mensaje de error desde el contenedor o desde la p�gina contenedora
		if (urlsDesconexion != undefined && urlsDesconexion != null) {
			...
			//Estoy en el contenedor, luego tengo que tomar los datos de la p�gina contenedora
			contenedora = false;
...
function obtenerIDContenedor(flujoId) {
	...
	try {
		//Se comprueba si se est� en la p�gina contenedora o en el contenedor
...
function mostrarMensajeIE(paramMensaje) {
	...
	if (paramMensaje[4] == 'false') {
		...
		if (paramMensaje[2] == 'null' || paramMensaje[2] == 0 || paramMensaje[2] == undefined) {
			...
			// El ancho de la ventana ser� el informado
	...
	if (paramMensaje[5] != null && paramMensaje[5] != "" && paramMensaje[5].toLowerCase() == "true") {
		...
		//ancho y alto de mensajes Dojo definidos en configuraci�n
		...
		try {
			if (paramMensaje[6] != null && paramMensaje[6] != "") {
				...
				if (frame != null) {
					...
					//Comprobamos si es el mismo idFlujo que ha lanzado la petici�n
					if (idFrame != paramMensaje[1]) {
						try {
							//Si el frame en el que nos encontramos tiene frames hijos
							//buscamos cual es el frame desde el que se lanzo la petici�n
							if (frame.ventanasEmergentes_Identificadores.length > 0) {
								for (var i = 0; i < frame.ventanasEmergentes_Identificadores.length; i++) {
									...
									if (i == frame.ventanasEmergentes_Identificadores[i]) {
										...
										//Si el flujoId obtenido es igual al que lanzo la petici�n es que hemos encontrado el frame
										//hijo que lanzo la petici�n
...
/**
 * Funcion que informa a trav�s del servlet de mensaje GUC
...
function informarServletGUCVentanaModal(ismostrarVentanaModal) {
	//Se comprueba que est� activo el servidor web
	...
	if (typeof ("VMODAL_CONTP") != undefined && VMODAL_CONTP != undefined && (VMODAL_CONTP == true || VMODAL_CONTP == "true")) {
		...
		try {
			if (paramMensaje[7] != 'null') {
				//almaceno el flujoId del padre del frame que lanzo la petici�n
				flujoIDOrigenMensajeDojo = paramMensaje[7];
				//almaceno el flujoId del frame que lanzo la petici�n
				flujoIDPadreMensajeDojo = paramMensaje[6];
				//variable que indica que se ha realizado la petici�n desde un frame hijo
				mensajeEnHijo = true;
			} else {
				// Se inicializa la varible con el flujoID del que ha hecho la petici�n Dojo.
		...
		oDialog._moveable = new dojo.dnd.Moveable(oDialog.domNode, {
			handle: oDialog.containerNode
		});
...
/*
 * Funci�n que se ejecuta cuando se cierra la ventana modal invocada con window.showModalDialog
 */
...
function fin(doc) {
	...
	if (flujoIDOrigenMensajeDojo != null && flujoIDOrigenMensajeDojo != "null") {
		...
		if (frame != null && frameId != null) {
			...
			try {
				//Se comprueba si se est� en la p�gina contenedora o en el contendor
...
//FORMATO
//CTAGS FASE II: Funci�n que inserta las celdas de datos en caso de informarse el atributo funcionformato.
//				 Esta funci�n s�lo se llama en caso de utilizarse las CTAGS.
...
function insertarControlHTML(tipoControl, nombre, fila, eventoSeleccionFila, camposEdicion, columnaSeleccionEdicion, funcionSeleccionFilas, seleccionCelda, deseleccionFila, opSegmentacion, funcionseleccion) {
	...
	//antes de las dobles comillas puedo insertar una funcion javascript m�s
...
// Anyadido par�metros filaspaginacion  segmentacion o Fix Ed-fila. No debe estar opSegmentacion
// Anyadido par�metro funci�n selecci�n para resaltado de fila
...
// Anyadido par�metros filaspaginacion  para segmentacion
...
//FORMATO
//CTAGS FASE II: Funci�n para el formateo por campos. Esta funci�n inserta el valor
//				 en la tabla una vez ha pasado por la funci�n de formateo.
...
function subirObjetosNegocio(claves, valores) {
	if (utils_isComuServidorWeb()) {
		...
		//Se comprueba que se realiza la ejecuci�n sobre el portal ATPI o ATPN
		try {
		...
		//Se comprueba si se est� ejecutando sobre el portal ATPN o sobre ATPI
		if (isATPI == true) {
			//Estamos en ATPI
...
//Funci�n para detectar la resoluci�n de la pantalla:
...
/**
Funcion que invoca una la creaci�n de una ventana modal
*/
...
//Funci�n que crea la ventana modal para internet explorer
...
/**
 * Funci�n que obtiene el padre, si existe, del frame desde el que se lanzo una ventana modal.
 */
...
/**
 * Determina si est� definido el plugin OCX.
 */
...
function ejecutarOperacionNACAREscenario(opNACAR, parametros, tipo, escenario, tipoEjecucion, descripcion) {
	if (utils_isComuServidorWeb()) {
		...
		if ((opNACAR != null) && (opNACAR != "") && (opNACAR != "null")) {
			...
			//Si se ha definido escenario y no operaci�n, activamos el escenario
...
//Funci�n que devuelve un valor booleano en funci�n de si la aplicaci�n se ejecuta en el Portal o no.
...
function ejecutarEscenarioNACAR(escenario, tipo) {
	//Llamamos a la funcion sin indicar operaci�n Nacar, de esta manera, ejecutar escenario
	//El par�metro tipoEjecucion lo ponemos a 0 que es el valor por defecto(funcionamiento sin backHistory)
	ejecutarOperacionNACAREscenario(null, null, tipo, escenario, "0", null)
}

/**
Funcion que invoca una la creaci�n de una ventana modal con t�tulo
*/
...
// Evolutivo lanzar url externa con t�tulo
...
/**
Funcion que invoca una ventana modal con t�tulo en FireFox
*/
function lanzarVentanaModalTituloFF(sURL, argumentos, sPropiedades, sTitulo, sScroll) {
	//Variable globales para informar los par�metros a la ventana modal
	var funcionRespuesta = "";
	if (argumentos != undefined) {
		if (sTitulo != undefined) {
...
/*
 * Funci�n encargada de obtener el dato que indica el ancho de la ventana modal
 */
...
/*
 * Funci�n encargada de obtener el dato que indica el alto de la ventana modal
 */
...
/*
 * Funci�n encargada de comprobar si existe la propiedad "textoProp" en la cadena "cadenaPropiedades" 
...
function insertarCeldaDatosEditable(nombre, fila, columna, eventoSeleccionFila, camposEdicion, columnaSeleccionEdicion, funcionSeleccionFilas, seleccionCelda, valor, seleccion, arrayValoresEdicion, protegida, arrayOpsSeg) {
	...
	// CTAGS FASE II OJO: para las celdas de datos no tiene sentido introducir la funci�n de desbloqueo de filas
...
//////////////////
// TABLA EDITABLE FASE II: edici�n en filas protegida
//////////////////
function desprotegerFilaSeleccionada(nombreTabla, numFilaSeleccionada, filaspaginacion) {
	...
	//ACTUALIZACI�N POR AGRUPACIONES
	...
	//FIN ACTUALIZACI�N POR AGRUPACIONES
		...
		if ((inputs != undefined) && (inputs != null)) {
			...
			// El radiobutton es un input
			{
				if (i - PRIMERA_FILA_DATOS == (numFilaSeleccionada % (filaspaginacion))) {
					...
					// err�tico del formulario con las celdas de
					// datos en edici�n en fila protegida con
	...
	// Para ejecutar el resto de las operaciones de selecci�n, debemos
	// devolver true.
	// Si devolvemos false, la selecci�n no se lleva a cabo.

	return true;
}

//Evolutivo Gesti�n Avanzada de Tareas
...
/*Funci�n invocada en el onload del formulario de una p�gina con FRAMES.
  Recuperar� de la p�gina los FRAMES que contiene y para cada uno obtendr�
  el identificador*/
function utils_IniciarPaginaFrames() {
	...
	if (utils_IsEjecucionPortal()) {
		for (var i = 0; i < window.frames.length; i++) {
			...
			//por lo que no se intentar� desconectar ni la instancia ni el flujo
			if (instancia != null) {
...
/* Funci�n que usar�n las aplicaciones cuando quieran informar al gestor de tareas la instancia-flujo
...
//Funci�n invocada por el unload del formulario de una p�gina con FRAMES o cuando la aplicaci�n lo encuentre oportuno.
...
function utils_recuperar_instancia_de_url(url) {
	...
	// Tambi�n se tratan otros distintos formatos
	var url_aux = "";
	try {
		url_aux = "" + url + "";
	} catch (err) {
	...
	// Si la url no indica el protocolo ni el host, se a�ade
	if ((rc != null) && (rc.search("/") == 0)) {
		rc = document.location.protocol + "//" + document.location.host + rc;
	}
	//si no tiene la palabra servlet, se devuelve null, porque no podr� ser usado como instancia.
	return rc;
}
...
function ejecutarEmulacionLocal() {
	//Comprobamos que medio de comunicaci�n con la arquitectura pesada debemos utilizar
	if (utils_isComuServidorWeb()) {
...
//Redirecciona a una URL agregando a la petici�n los datos obtenidos del escritorio pesado
...
//ACTUALIZACI�N POR AGRUPACIONES
/***
 *Funciones para la inserci�n de datos de la tabla con agrupaciones.
 ****/
...
/**
 * Funci�n que activa la ventana modal
 */
function utils_activarVentana(url, id, ObjPortal, nacCtx, invocador) {
	...
	// Gesti�n del Foco en ventanas modales

	// Se inserta el evento onFocus en la ventana padre. La funci�n despacharFocoAVentanaModalFF() ser�
	...
	if (this.addEventListener) { // Mozilla, Netscape, Firefox
		this.addEventListener("focus", function () {
			despacharFocoAVentanaModalFF(id)
		}, true);
	} else { //IE
	...
	if (invocador != undefined && invocador != null && invocador != "") {
		if (invocador != "MostrarMensaje") {
			//alert(invocador);
			setTimeout(function () {
				gestionTareaModalesFF(ObjPortal, id);
			}, 1000);
		}
	}
...
/*
 *  Funcion ejecutada cuando se cierra la ventana modal y que ejecuta en el caso de que la ventana haya sido creada desde la funci�n mostrarMensajeNN
 *  el c�digo ejecutado cuando se recibe el resultado de la ventana modal
 *  Esta funci�n solo se invoca cuando el navegador es FireFox.
 */
function mostrarResVentanaModal(variableRetorno, invocador) {
	...
	if (invocador == "MostrarMensaje") {
		...
		if (variableRetorno == undefined) {
			// Si la variable de retorno viene undefined se puede recalcular para el caso de mensajes utilizando el valor de los par�metros de mensajes
		...
		if (variableRetorno != null && variableRetorno != "") {
			...
			//Se comprueba si se ha definido alg�n valor de retorno para la ventana que invoc� a la ventana modal
			if (myValorRetornado) {
				//En el caso de que la variable est� definida
...
function crearVentanaModalConvencionalFirefox(titulo, iconoCerrar, literalCerrar, id, scroll, url, newAncho, newAlto, parametros, variableRetorno, invocador, funcionRespuesta, flujoId, nacarC) {
	...
	if (newAlto != undefined && newAlto != null && newAlto != 0 && newAlto != "0") {
		...
		//Si no se han definido un alto, se a�aden 2 p�xeles para centrar la ventana
		factorCorreccion = 2;
	}
	...
	//Se calcula el tama�o del frame interno. En el alto, se descuenta el tama�o de la cabecera.
	...
	/*Creamos la cadena con que generar� la ventana modal*/
	...
	/*Final de la creaci�n de la cadena con que generar� la ventana modal*/
...
function cerrarVentanaModalFF(id, datos, invocador, funcionRespuesta, idFrame) {
	...
	if (invocador != undefined && invocador != null && invocador != "") {
		//Se recupera el frame que invoc� a la ventana Modal (en caso de existir)
...
//Funci�n que actualiza el nacarContext de la ventana
...
/*
 *  Funci�n ejecutada para ocultar y eliminar la ventana modal, est� funci�n es ejecutada cuando se cierra la ventana modal
 */
...
/**
 * Funci�n que crea la cadena que generar� la capa que bloquea el resto de la ventana.
 */
...
// M�todo que informa al gestor de la nueva instancia y tarea que se est� ejecutando.
...
//Funciones para la obtenci�n y asignaci�n de par�metros en las ventanas modales

//Funci�n que devuelve los par�metros pasados a la ventana modal.
...
//Funci�n que obtiene el nacarContext en la ventana modal
...
//Funci�n que asigna la respuesta que va a devolver la ventana modal
...
// Evoluci�n para validar si una serie de operaciones pesadas est�n autorizadas
// Genera una mensaje a partir del array de operaciones que recibe la funci�n como par�metro.
// Utiliza tambi�n un nuevo tipo de mensaje para comunicarse con el plugin o el servidor web local.
function isOperacionPermitidaEscritorio(arrayOperaciones) {
	...
	if (isEjecucionPortal()) {
		...
		if (arrayOperaciones != null && arrayOperaciones.length > 0) {
			// El formato ser� operacion1*+*operacion2*+*operacion3 ...
			for (var i = 0; i < arrayOperaciones.length; i++) {
				if (i > 0) {
					mensaje = mensaje + "*+*";
				}
			...
			if (utils_isComuServidorWeb()) {
				// Utiliza el servidor Web Local para ejecutar la petici�n sobre la arquitectura.
				retorno = utils_servidorWeb_enviaPlanoSinc(mensaje);
			} else if (isNavegadorEmbebido()) {
				// Utiliza el plugin para ejecutar la petici�n sobre la arquitectura
...
// Trata el resultado obtenido a trav�s de la invocaci�n a la arquitectura. Recibe un mensaje
...
// operaciones est�n autorizadas o no.
...
Diferencia por navegador dado que si el navegador es FireFox la funci�n window.close() no tiene sentido pues se utiliza 
una soluci�n por capas javaScript
*/
...
// en funci�n del parametro bSoloListaOps.
...
//Activaci�n/desactivaci�n del cierre controlado desde pulsaci�n de Aspa del escritorio pesado
function activarCierreControladoAspa(activar) {
	...
	//env�o de la petici�n asincrona y sin esperar respuesta
...
//Implementaci�n del cierre controlado desde pulsaci�n de Aspa del escritorio pesado
...
/**
Funcion que invoca una la creaci�n de una ventana modal con paso de parametros por Get
*/
...
//Funci�n que crea la ventana modal para internet explorer con paso de parametros por Get
...
/**
Funcion que invoca una la creaci�n de una ventana modal con t�tulo y con paso de parametros v�a get
*/
...
// Funci�n que crea la ventana modal con titulo para internet explorer con paso de parametros v�a Get
...
// Funci�n que despacha el foco a la ventana modal.
...
function asignarRespuestaVentanaModalContenedoresPesados(respuesta) {
	...
	//Ejecutamos petici�n para comprobar si la operacion es ligera o pesada
...
function utils_lanzarProcesoCerrarVentanaModal(parametros, funcionRespuesta) {
	if (ie) {
		...
		//Si hay funci�n de respuesta se ejecuta para obtener los parametros de la ventana modal desde la ventana padre
...
/**
 * Ejecuta la petici�n de Login a MicroStrategy, retornando el sessionState de la sesi�n abierta 
 */
...
/**
 * Ejecuta la petici�n de ReportExecute en una ventana modal 
 */
...
/**
 * Ejecuta la petici�n de Logout a MicroStrategy, retornando el sessionState de la sesi�n abierta 
 */
...
/* Control MOMT (M�ltiples Operaciones en la Misma Tarea). Lanza la ejecuci�n de una nueva operaci�n ligera conservando la tarea */
function ejecutarOperacionMismoNavegador(opNACARP, opNACARL, parametros, descripcion) {

	if (utils_IsEjecucionPortal()) {
		if (opNACARL != null && opNACARL != "" && opNACARL != "null") {

			try { //FIX: si no est� subida la versi�n de ATPI que incluye este Ev, no se hace nada
