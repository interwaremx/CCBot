/* Librería de funciones para aplicar/desaplicar mascaras por defecto en función del locale activo a fechas e importes. */
...
function aplicaMascaraFechaDefecto(p1,p2,p3,p4){
	/*uno/dos parámetros: valor e indice*/
	...
	/*tres/cuatro parámetros: dia, mes, año e indice*/
...
/* Evo. Selección Locale en Mascaras Multiidioma*/

function aplicaMascaraFechaLocaleSelec(p1,p2,p3,p4,p5){
	/*un parámetro: valor, se aplicará la máscara por defecto*/
	...
	if(p1!=undefined && p2!=undefined && p3!=undefined && p4!=undefined && p5==undefined){
		...
		/*cinco parámetros: dia, mes, año, locale e indice*/
	if(p1!=undefined && p2!=undefined && p3!=undefined && p4!=undefined && p5!=undefined){
		var anyo=""+p3;
		var mes=""+p2;
		var dia=""+p1;
...
/* Realiza una peticion AJAX síncrona */
...
/* Carga dinámica, en la cabecera de la página, de la librería Dojo recuperada síncronamente por AJAX */
...
/* Invocación a la funcion de aplicación de mascaras, previa carga dinámica de Dojo si es necesario */
...
/* Función de aplicación/desaplicación de máscaras por defecto */
function arq_mm_mascarasMultiidioma(op,obj,valor,indice,locale){
	...
	/* configuración del locale de dojo */
	...
	try{
		if(obj==0){//fechas
			...
			if(mascara != null && valor != null && locale_dojo != null){
				if(op==0){//aplicar
					...
					// Comprabamos que el mes esté comprendido entre 12 y 1
					if((mes)>12 || mes==00){
							return -1;				
					}
					...
					//Tratamos si el año es bisiesto, calculando si el resto de las divisines es 0
					...
					//Comprobamos que el año no es bisiesto					
				  if(mes == 2 && !((anyo % 4 == 0 && anyo % 100 != 0) || anyo % 1000 == 0)) {
					...
					fecha = dojo.date.locale.format(new Date(anyo,mesaux,dia,0,0,0), {datePattern: mascara, selector: "date", locale: locale_dojo});
					if (anyo>=0 && anyo<=1900){
						if (mascara.lastIndexOf("yyyy")!=-1){
							var auxPosicion = null;
							//Obtengo la posición del año en la máscara para colocarlo en la misma posición
							auxPosicion=mascara.indexOf("y");
							//Obtengo el año después de aplicar la máscara
							fecha_FormateadaAux=fecha.substr(auxPosicion,4);
							//De la fecha inicial, me quedo con el año							
							var valorAux=valor.substr(0,4);
							//Sustituimos el año que redonda Dojo por el año original tratado en el paso anterior en función de la máscara seleccionada
				...
				if(op==1){//desaplicar
					var f = dojo.date.locale.parse(valor,{datePattern: mascara, selector: "date", locale: locale_dojo});					
					var res = ""+f.getYear();
					//Redondeo de ceros para equiparar la máscara con valor desformateado.
					if (mascara.lastIndexOf("yyyy")!=-1){				
		...
		if(obj==1){//numeros
			...
			if(mascara != null && valor != null && locale_dojo != null){
				if(op==0){//aplicar
					valor = valor.split(",").join(".");//transformación al formato estandar dojo
					var res = ""+dojo.number.format(valor, { pattern: mascara, locale: locale_dojo  });
					return arq_mm_eliminaCero(mascara,res,valor);
				}
				if(op==1){//desaplicar

					var res = ""+dojo.number.parse(valor, { pattern: mascara, locale: locale_dojo  });
				  var quitarCerosFin=new Boolean(true);
				  var posicValida=valor.length;
				  
					//Si el valor no se corresponde con la máscara, se devuelve el valor original
					if (res=="NaN"){
						//Si es necesario, se eliminan los ceros situados a la derecha en la parte decimal 
						//(para igualar comportamiento con máscaras java).
						if(valor.indexOf(",")>=0){
					...
					res = res.split(".").join(",");//transformación al formato estandar Nacar
					return res;
				}
			}else
				return "ERROR02";
...
/* eliminación del cero para importes entre -1 y 1 y mascaras de tipo #.00 */
