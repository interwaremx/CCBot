...
EventList[0]=new Array(2);
 ...
 var _TP_CIFRA        = 5;    // números sin nigún tipo de separador
...
//realizamos esto para asegurarnos que el scroll sólo aparece si es necesario.
...
document.body.style.overflow='auto';
	...
	*****																									*****
	***** Nombre 	  : ToggleRadio                      											    	*****
	***** Descripción : Establece la fila seleccionada						 							    ***** 
	***** Parámetros  : 																					*****
	*****				e 		= radio																		*****
	*****				numFila = número de fila en la que estamos											*****  
	*****																									*****
	*************************************************************************************************************/
	...
	*****																									*****
	***** Nombre 	  : Unhighlight                      											    	*****
	***** Descripción : Desilumina la fila anteriormente seleccionada		 							    *****
	***** Parámetros  : 																					*****
	*****				e 		= radio																		*****
	*****				numFila = número de fila en la que estamos											*****  
	*****																									*****
	***** NOTA		  : Se deberá de crear una función cambioColor(numfila,pijama), de igual manera que la  *****
	*****				creada para la aplicación NCTC, que haga refencia al nombre de las columnas que 	*****
	*****				tenga la trabla																		*****
	*****																									*****
	*************************************************************************************************************/
	...
	*****																									*****
	***** Nombre 	  : Highlight	                      											    	*****
	***** Descripción : Ilumina la fila pasada como parametro						 						*****
	***** Parámetros  : 																					*****
	*****				e 		= radio																		*****
	*****				numFila = número de fila en la que estamos											*****  
	*****																									*****
...
{
	///
	//la funcion validar depende del nombre del campo 
	//por lo que  hay que usar correctamente la nomenclatura
	///
 ...
 *              oHdValorCombo:     objeto imput hidden que se actualizará con el valor seleccionado del combo
 *******************************************************************************************************************/
 ...
 **********************************************************************************************************************/
 ...
 function limpiarTabla(idTabla,numfilasCabeceras) 
 {
    ...
    // si numfilasCabeceras==0, se trata de una tabla sin cabecera.
    ...
    for ( var i=numfilasCabeceras; i < objTabla.rows.length; i++ ) 
    {
        ...
        //objCelda = objTabla.cells[0];
        //objFila.setAttribute('height', '21');
 ...
 ***************************************************************************************************************/
 ...
 /******************************************************************************************************************
 * Comprueba si la validacion del valor del campo fué correcta, asignandole la apariencia en cosecuencia.
 * 
 *  PARAMETROS:
 ...
 *******************************************************************************************************************/
 ...
 function aspectoFinalComp (isOk, objeto, oblig, tpCampo, mensaje)
 {
    ...
    // si el campo NO es opcional y NO está vacio, llamara la función
    //    
    else
 ...
 * 
 *  PARAMETROS:
 *              iTpDato    -> Flag númerico que representa un tipo de dato:
 *                             0.- Numero        2.- Fechas       4.- texto (excluye texto no aplicable)
 *                             1.- Alfanum       3.- Decimal      5.- porcentaje
 *                             6.- importes (incluye nº negativos)7.- numeros(sin puntos ni comas)
 *			       8.- email
 *******************************************************************************************************************/
 ...
 //var e =window.event 
    ...
    switch (TP_DATO)
    {
       case _TP_CIFRA     : sPatron = /^[0-9]*$/;                             break;
       case _TP_NUMERICO  : sPatron = /^[.]*[0-9]*$/;                         break;
       case _TP_FECHA     : sPatron = /^[0-9]*[/]*$/;                         break;
       case _TP_DECIMAL   : sPatron = /^[.]*[,]*[0-9]*$/;                     break;
       case _TP_TEXTO     : sPatron = /^["'"^'>'^'<'^'%'^'"'^"`"^"^"^"\\"]+$/;break;	                   
       case _TP_PORCIENTO : sPatron = /^[,]*[0-9]*$/;                         break;
       case _TP_DECIMAL_NN: sPatron = /^[-]*[.]*[,]*[0-9]*$/;                 break;      
    }


    // Si es INTRO y buscar está habilitado llama a la funcion que lanza el evento
    ...
    // Si es texto, se pone en mayúscula    
    //if ( _TP_TEXTO == TP_DATO )
     //   tecla = sCadena.toUpperCase().charCodeAt(0);
 }
...
/******************************************************************************************************************
 * estaVacio,  Valida que la cadena este vacia. Devuelve True si está vacia.
 *
 *  PARAMETROS:
...
/** **********************************************************************
 * comprueba que el codigo postal sea valido                                 <br>
 *                                                                                                                 <br>
 * @param oCp Código postal                                                             <br>
 * @return  bOblig Indicador de obligatoriedad                                 <br>
 *********************************************************************** */
function esCpValido (oCp, bOblig)
{
    ...
    // comprueba que esté relleno de numeros   
    ...
    if (!bIsOk) sMsj = "Código postal erróneo";
 ...
 *
 *  PARAMETROS:
 *              iTotalFilas        -> Número de filas total establecido de la lista (filas a mostrar)
 *              iTotalFilasDatos   -> Número total de filas de la tabla con datos (total filas tabla datos).
 *              vectColum          -> Vector con el ancho de cada columna
 *              iFilasxReg         -> Número de filas en la tabla por registro de datos. CAMPO NO OBLIGATORIO
 *              iFilasXPagina      -> solo para paginacion x javascript. Numero de registros de cada páginacion
 *****************************************************************************************************************/
 
 function insertarFilasEnBlancoDatos (iTotalFilas, iTotalFilasDatos, vectColum, sNomFila, iFilasXPagina, iNumFilasxReg)
 {

    // si es undefined, es que no se está pasando este argumento. 
    var iFilasxReg = (iNumFilasxReg == undefined? 1 : iNumFilasxReg);
		var var_tabla = document.getElementById("tbodyDatos");

    // oculta las filas en blanco insertadas si hay mas filas a visualizar que las que se mostrará por pantalla.
   ...
   // Si el numero de filas establecido es menor o igual al numero de filas de la tabla
    // de datos, no será necesario insertar nuevas filas vacias. 
   if(parseInt(iTotalFilasDatos)!=0)
   {
    ...
    for(i = 0; i < parseInt(numFilas); i++)
    {
        ...
        {
            // añade el nombre a la fila, para poder ocultarla o hacerla visible
            ...
            for (indColAux=0 ; indColAux < vectColum.length ; indColAux++)
            {
            	...
            	var_columna.setAttribute('width', vectColum[indColAux] + '%');
           		//var_columna.setAttribute('class', Pijama);
           		var_columna.innerHTML="";
         		  var_fila.appendChild(var_columna);
            }
            // cierra la fila añadida a la tabla
